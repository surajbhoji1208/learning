https://codeinterview.io/DEMO-BJSNMWYGTF
...
What is ng-template?
=> A hidden Angular template that is not rendered until Angular explicitly creates a view from it.

What is TemplateRef?
=> A reference to an Angular template (ng-template) that can be instantiated later.

What is ViewContainerRef?
=> A container used to insert, remove, and manage views dynamically.

Why are TemplateRef and ViewContainerRef used together?
=> TemplateRef contains the template and ViewContainerRef renders that template into the DOM.
...
=> In Angular, change detection is the mechanism that checks for changes in component data and updates the DOM accordingly.
By default, Angular uses ChangeDetectionStrategy.Default, which means it runs change detection for the entire component tree whenever any event (like click, HTTP response, timer, input change) occurs.

How does Angular's change detection mechanism work?
=>Change Detection Flow
Event happens (click, HTTP response, timer, etc.)
Zone.js notifies Angular
Angular runs change detection starting from root
Components are checked one by one
If a binding value changed → Angular updates the DOM
If not changed → Skips DOM update (but still runs checks)
...
use when When you don’t care about past values, only future ones
************eg********************
* const subject = new Subject() ;
* subject.next(1) ;
* subject.next(2) ;
* subject.subscribe(value => console.log("Subscriber:", value)) ;
* // Output: (nothing, because 1 & 2 already emitted)
* subject.next(3) ;
* // Output: Subscriber: 3

BehaviorSubject
=>✅ Must have an initial/default value
✅ Always stores the latest value
Subscriber immediately receives last emitted value (even if emitted before subscription)
use When you need current state + future updates (like form state, logged-in user state, etc.)
****************eg****************
const behavior = new BehaviorSubject(0) ; // initial value: 0

behavior.next(1) ;
behavior.next(2) ;

behavior.subscribe(value => console.log("Subscriber:", value)) ;
// Output immediately: 2 (last emitted value)
...
Order 	Executes one at a time, sequential	Executes all at once, parallel
Output Order 	Preserves order 	Does NOT guarantee order
Use Case	 API calls in Order 	API calls in parallel (faster, no dependency)
Concurrency 	1 at a time 	Unlimited by default
...
from([1, 2, 3]).pipe(
concatMap(id => apiCall(id))
)

With mergeMap
➡ A, B, C start at the same time
➡ Whichever finishes first is emitted first
✅ Faster, unordered
...
When to Use What?
Situation	 Use
Button clicks, event streams Subject
App state, current user, selected item	 BehaviorSubject
Chat messages, notifications history, caching API data 	ReplaySubject
API calls that must run in order	 concatMap
API calls that can run in parallel 	mergeMap
